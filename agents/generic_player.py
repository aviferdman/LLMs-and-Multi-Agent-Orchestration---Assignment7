...
from agents.player_handlers import handle_invitation, handle_parity_call, handle_game_over
...
from SHARED.constants import (HTTP_PROTOCOL, LOCALHOST, MCP_PATH, SERVER_HOST,
                              Field, LogEvent, MessageType, Status, StrategyType)
...
class GenericPlayer:
        ...
        async def mcp_endpoint(request: Request) -> JSONResponse:
            try:
                ...
                if msg_type == MessageType.GAME_INVITATION:
                    response = handle_invitation(self.player_id, message, self.logger,
                                                 message.get(Field.CONVERSATION_ID))
                elif msg_type == MessageType.CHOOSE_PARITY_CALL:
                    response = handle_parity_call(self.player_id, message,
                                                  self.strategy, self.logger)
    ...
    async def _register_with_lm(self):
        try:
            ...
            resp = await send_with_retry(_lm_endpoint, msg,
