...
This module handles incoming messages from League Manager and Players,
delegating match operations to the appropriate handlers.
"""
...
def handle_round_announcement(message: dict, referee, background_tasks, request_id: Optional[int] = None) -> dict:
    ...
    my_matches = [m for m in matches if m.get("referee_endpoint") == referee.endpoint]

    referee.logger.log_message(
        "ROUND_ANNOUNCEMENT_RECEIVED",
        {"round_id": round_id, "total_matches": len(matches), "my_matches": len(my_matches)},
    )
    ...
    for match in my_matches:
        ...
        if not player_a_endpoint or not player_b_endpoint:
            referee.logger.log_error(LogEvent.ERROR, f"Cannot resolve endpoints for {player_a_id} or {player_b_id}")
            continue

        referee.logger.log_message(
            "STARTING_MATCH",
            {Field.MATCH_ID: match_id, Field.PLAYER_A_ID: player_a_id, Field.PLAYER_B_ID: player_b_id},
        )

        background_tasks.add_task(
            referee.run_match, league_id, round_id, match_id, player_a_id, player_b_id, player_a_endpoint, player_b_endpoint
        )
...
def handle_game_join_ack(message: dict, referee, request_id: Optional[int] = None) -> dict:
    ...
    if match_id in referee.active_matches:
        ...
        referee.logger.log_message("PLAYER_JOINED", {Field.MATCH_ID: match_id, Field.PLAYER_ID: player_id})
...
def handle_parity_choice(message: dict, referee, request_id: Optional[int] = None) -> dict:
    ...
    if match_id in referee.active_matches:
        ...
        referee.logger.log_message(
            "CHOICE_RECEIVED", {Field.MATCH_ID: match_id, Field.PLAYER_ID: player_id, Field.PARITY_CHOICE: choice}
        )
    result = {Field.STATUS: Status.OK}
    return _wrap_ack(result, request_id)
