...
from SHARED.contracts import (build_league_completed, build_league_standings_update,
                              build_round_announcement, build_round_completed)
from SHARED.league_sdk.repositories import StandingsRepository


async def run_league_matches(league_config, registered_players: Dict[str, Any],
                             registered_referees: Dict[str, Any], logger, state: Dict[str, Any]):
    ...
    try:
        ...
        logger.log_message("SCHEDULE_LOADED",
            {"total_rounds": total_rounds, "total_matches": sum(len(r) for r in schedule)})
        round_results: List[Dict[str, Any]] = []
        for round_num, round_matches in enumerate(schedule, start=1):
            state["current_round"] = round_num
            current_round_results = await _execute_round(round_num, total_rounds, round_matches,
...
async def _execute_round(round_num: int, total_rounds: int, round_matches: list, league_config,
                         registered_players, registered_referees, logger, state) -> List[Dict[str, Any]]:
    """Execute a single round of matches."""
    announcement = build_round_announcement(league_config.league_id, round_num, total_rounds,
    ...
    for match in round_matches:
        result = await execute_match_via_referee(match, league_config, registered_players,
...
async def _send_round_completed(round_num: int, results: list, league_config,
                                registered_players, registered_referees, logger):
