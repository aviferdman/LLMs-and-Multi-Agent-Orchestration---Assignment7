...
def test_valid_state_transitions():
    ...
    assert sm.transition(MatchState.COLLECTING_CHOICES) and sm.current_state == MatchState.COLLECTING_CHOICES
    assert sm.transition(MatchState.DRAWING_NUMBER) and sm.current_state == MatchState.DRAWING_NUMBER
    assert sm.transition(MatchState.FINISHED) and sm.current_state == MatchState.FINISHED

def test_invalid_state_transitions():
    """Test invalid state transitions are rejected."""
    sm = MatchStateMachine()
    assert not sm.transition(MatchState.FINISHED) and sm.current_state == MatchState.WAITING_FOR_PLAYERS
